...
async function getData() {
  const [vehiclesRes, newsRes, manufacturersRes, siteConfigRes, comparisonsRes, categoriesRes, sectionsRes] = await Promise.all([
    supabase
      .from('vehicles')
...
export default async function HomePage() {
  const { vehicles, news, manufacturers, upcoming, siteConfig, comparisons, categories, sections, vehicleCount, manufacturerCount, newsCount } = await getData();
  ...
  // Build a map of enabled sections in their saved order
  const enabledSections = (sections || []).filter(s => s.is_enabled);
  const sectionMap = new Map(enabledSections.map(s => [s.section_key, s.sort_order]));
  const isSectionEnabled = (key: string) => sectionMap.has(key);
  ...
  const renderSection = (key: string): React.ReactNode => {
    ...
    switch (key) {
      ...
      case 'stats':
        return (
          <section key={key} className="bg-[#0a2e14] border-t border-white/10 relative overflow-hidden">
  ...
  // Render sections in the order saved in the database
  const orderedKeys = enabledSections
